...
class APIService: APIServiceProtocol {
    ...
    #if DEBUG
    private static let debugLogQueue = DispatchQueue(label: "AgentMeter.APIService.debugLogQueue")
    #endif
    ...
    private func appendDebugLog(data: Data, response: URLResponse, source: String? = nil) {
        ...
        let entry = "[\(Date())] \(sourceLabel)Status: \(statusCode), bytes: \(data.count)\n"
        ...
        Self.debugLogQueue.sync {
            ...
            let debugURL = baseDirectory.appendingPathComponent("agentmeter_debug.txt")
